# K-fold CV for partial stringency model
...
# Lag in the model (i.e. no masking used)
lag = 60
...
# Set lambda search parameters
l1_lamb = [1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1]
n_lamb = len(l1_lamb)

# Set learning rate search parameters
...
for i in range(n_lamb):
	for j in range(len(lr)):
		...
		for k in range(K):
			...
			train_list = [stringency_tensor_3D[train_idx, :, :]]
			y = f_t_tensor[train_idx, :]
			# Validation set
			val_list = [stringency_tensor_3D[val_idx, :, :]]
		...
		val_avg = np.mean(val_mat, axis=0)
		if i == 0 and j == 0:
...
for i in range(n_lamb*len(lr)):
	plt.plot(epoch_list[10:], kf_results.iloc[int(i*n_measure):int((i+1)*n_measure), 3][10:])
...
# Key              30000-Lambda-0.05-LR-0.0005
# Epoch                                  18500
# Average Train                       0.018901
# Average Val                          0.01872

# Save K-fold results dataframe
kf_results.to_csv(os.getcwd()+'/results-cv/kf_results_stringency.csv', header=True, index=False)

# Plot the stringency index over the study period
plt.plot(stringency_tensor)
plt.xlabel("Days in Study Period")
plt.ylabel("Stringency Index Level")
plt.title("Stringency Index Over Study Period")
plt.show()