# K-fold CV for joint model (stringency and temp)
...
#######################
random.seed(8)
# Read in contact factor, stringency index plus temperature covariates
# Alignment dates
...
for i in range(n_lamb):
	for j in range(len(lr)):
		...
		for k in range(K):
			...
			train_list = [stringency_tensor_3D[train_idx, :, :], sma14_temp_tensor_3D[train_idx, :, :]]
			y = f_t_tensor[train_idx, :]
			# Validation set
			val_list = [stringency_tensor_3D[val_idx, :, :], sma14_temp_tensor_3D[val_idx, :, :]]
			y_val = f_t_tensor[val_idx, :]
			# Set up model
			fn_model = functional_nn.fnNetwork_fhat2_vecbeta(lag, weight_dim, init_vals)
...
kf_results.to_csv(os.getcwd()+'/results-cv/kf_results_strin_temp.csv', header=True, index=False)